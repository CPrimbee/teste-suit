<div>
    <x-modal :title="__('Update Course: #:id', ['id' => $form->curso?->id])" wire
        x-on:open="setTimeout(() => $refs.name.focus(), 250)" size="4xl" blur center persistent>
        <form id="course-update-{{ $form->curso?->id }}" wire:submit="save"
        ...
        <x-slot:footer>
            <x-button type="submit" form="course-update-{{ $form->curso?->id }}" loading="save">
                @lang('Save')
            </x-button>
        </x-slot:footer>
    </x-modal>
